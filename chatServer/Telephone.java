...
public class Telephone {
    private static String regex = "^(\\+7|7|8)?[\\s\\-]?\\(?[489][0-9]{2}\\)?[\\s\\-]?[0-9]{3}[\\s\\-]?[0-9]{2}[\\s\\-]?[0-9]{2}$";
    private static Pattern pattern = Pattern.compile(regex);

    public static String processTelephone(String telephone) throws IncorrectTelephoneException {
        ...
        if (match.find()) {
            ...
            throw new IncorrectTelephoneException("Incorrect telephone");
        }
    }
}
